...
async function atualizarDashboard(ano, mes) {
    ...
    for (let i = 1; i <= ultimoDiaDoMes; i++) {
        ...
        if (registroParaEsteDia) {
            ...
            // Se for exceção, meta para o dia é 0, então não adiciona nada
        } else {
    ...
    if (anoVisualizado === dataAtualCalculo.getFullYear() && mesVisualizado === (dataAtualCalculo.getMonth() + 1)) {
        for (let i = dataAtualCalculo.getDate() + 1; i <= ultimoDiaDoMes; i++) {
            ...
            if (!registroParaEsteDia || !registroParaEsteDia.isExcecao) { // Se não foi lançado ou não é exceção
        ...
        // Se o mês visualizado não é o mês atual, conta os dias úteis que não foram lançados como exceção
        ...
        diasRestantesMensal = diasUteisNaoLancados - diasDeOperacaoConsideradosMensal; // Subtrai os dias já considerados
    }
