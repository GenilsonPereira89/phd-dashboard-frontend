...
const metaMensalPadraoElement = document.getElementById('metaMensalPadrao'); // NOVO SELETOR
const metaRealDoMesElement = document.getElementById('metaRealDoMes'); 
...
function getDiasDeOperacaoNoPeriodo(dataInicio, dataFim, producoes) {
    ...
    while (current <= finalDate) {
        ...
        // Se não for domingo E não for exceção (seja por registro ou por padrão)
        // OU se for domingo E houver registro E não for exceção (domingo trabalhado)
...
async function atualizarDashboard(ano, mes) {
    ...
    // --- CÁLCULO DA META MENSAL (PADRÃO) --- NOVO CÁLCULO
    let metaMensalPadraoCalculada = 0;
    ...
    let diasUteisNoMesParaMetaPadrao = 0; // Dias que não são domingo e não são exceção
    const tempDateForStandardMeta = new Date(primeiroDiaDoMes.getTime());
    while (tempDateForStandardMeta <= ultimoDiaDoMes) {
        const dataStringTemp = formatDateToYYYYMMDD(tempDateForStandardMeta);
        const registroParaEsteDia = producoesDoMesVisualizado.find(p => p.data === dataStringTemp);
        const dayOfWeekTemp = tempDateForStandardMeta.getDay(); // 0 = Domingo

        // Conta dias que não são domingo E não são exceção (seja por registro ou por padrão)
        // OU se for domingo E houver registro E não for exceção (domingo trabalhado)
        if ((dayOfWeekTemp !== 0 && !(registroParaEsteDia && registroParaEsteDia.isExcecao)) ||
            (dayOfWeekTemp === 0 && registroParaEsteDia && !registroParaEsteDia.isExcecao)) {
            diasUteisNoMesParaMetaPadrao++;
        }
        tempDateForStandardMeta.setDate(tempDateForStandardMeta.getDate() + 1);
    }
    metaMensalPadraoCalculada = diasUteisNoMesParaMetaPadrao * NUM_OPERADORES_PADRAO * PACOTES_POR_OPERADOR_DIA_META;

    if (metaMensalPadraoElement) {
        metaMensalPadraoElement.textContent = metaMensalPadraoCalculada.toLocaleString('pt-BR');
    }


    // --- CÁLCULO DA META REAL DO MÊS (DINÂMICA) --- (EXISTENTE)
    let metaRealDoMesCalculada = 0; 
    let diasDeOperacaoConsideradosNaMetaReal = 0; 
    ...
    while (tempDateForMetaReal <= ultimoDiaDoMes) {
        ...
        const registroParaEsteDia = producoesDoMesVisualizado.find(p => p.data === dataStringTemp);
        const dayOfWeekTemp = tempDateForMetaReal.getDay(); 
        ...
        if (registroParaEsteDia) {
            if (!registroParaEsteDia.isExcecao) {
                ...
                diasDeOperacaoConsideradosNaMetaReal++; 
            }
        } else {
            if (dayOfWeekTemp !== 0) { 
    ...
    producoesDoMesVisualizado.forEach(registro => {
        ...
        if (!registro.isExcecao) {
            ...
            if (dataRegistroObj <= dataAtualCalculo) {
                ...
                if ((dayOfWeekRegistro !== 0 && !registro.isExcecao) || (dayOfWeekRegistro === 0 && !registro.isExcecao)) {
    ...
    while (tempDateForRemainingDaysMonthly <= lastDayOfMonth) {
        ...
        // Se não há registro E não for domingo
        // OU se há registro E não for exceção E não for domingo
        // OU se for domingo E há registro E não for exceção (domingo trabalhado)
        if ((!registroParaEsteDia && dayOfWeekTemp !== 0) || 
            (registroParaEsteDia && !registroParaEsteDia.isExcecao && dayOfWeekTemp !== 0) ||
            (dayOfWeekTemp === 0 && registroParaEsteDia && !registroParaEsteDia.isExcecao)) {
