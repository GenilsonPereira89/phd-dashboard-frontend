...
// NOVOS SELETORES PARA O RELATÓRIO COMPARATIVO
...
const exportarRelatorioCsvBtn = document.getElementById('exportarRelatorioCsvBtn'); // NOVO SELETOR
...
// NOVA FUNÇÃO: Buscar produções por período
async function fetchProducoesByPeriodAPI(dataInicio, dataFim) {
    try {
        ...
        if (!response.ok) {
            throw new Error(`Erro ao buscar produções por período: ${response.statusText}`);
        }
        const data = await response.json();
        return data.map(item => ({
            data: item.data,
        ...
        console.error('Erro ao carregar produções por período da API:', error);
        alert('Erro ao carregar dados de produção para o relatório. Verifique o servidor backend.');
        return [];
    }
}
...
// Popula os seletores de mês para o relatório comparativo
function popularSeletoresDeMesRelatorio() {
    const meses = [
        "Janeiro", "Fevereiro", "Março", "Abril", "Maio", "Junho",
        "Julho", "Agosto", "Setembro", "Outubro", "Novembro", "Dezembro"
    ];
    ...
    // Define o mês atual como padrão para o seletor de fim
    const hoje = new Date();
...
// NOVO EVENT LISTENER: Gerar Relatório Comparativo
gerarRelatorioComparativoBtn.addEventListener('click', async () => {
    ...
    const ano = anoVisualizado; // Usa o ano atualmente visualizado no dashboard principal
    ...
    for (let currentMonth = mesInicio; currentMonth <= mesFim; currentMonth++) {
        const dataInicioMes = new Date(ano, currentMonth - 1, 1);
        const dataFimMes = new Date(ano, currentMonth, 0); // Último dia do mês
...
function exportTableToCSV(filename) {
    ...
    ultimoRelatorioGerado.kpis.forEach(kpi => {
        ...
        ultimoRelatorioGerado.resultados.forEach(mesResult => {
            let valor = mesResult[kpi.id];
            if (kpi.format === 'number') {
                valor = valor; // Números já estão como números, não precisam de toLocaleString para CSV
            } else if (kpi.format === 'decimal') {
...
// NOVO EVENT LISTENER: Botão de Exportar CSV
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    popularSeletoresDeMesRelatorio(); // Chama a nova função para popular os seletores do relatório
