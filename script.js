...
async function atualizarDashboard(ano, mes) {
    ...
    const tempDateForRemainingDays = new Date(today.getTime());
    tempDateForRemainingDays.setHours(0,0,0,0); // Garante que a hora seja 00:00:00

    if (tempDateForRemainingDays < primeiroDiaDoMes) { // Se o mês visualizado é futuro
        tempDateForRemainingDays.setTime(primeiroDiaDoMes.getTime());
    } else if (tempDateForRemainingDays > lastDayOfMonth) { // Se o mês visualizado já passou
        diasRestantesParaProjecao = 0;
    } else { // Mês atual
        tempDateForRemainingDays.setDate(today.getDate() + 1); // Começa a contar a partir de amanhã
        tempDateForRemainingDays.setHours(0,0,0,0); // Zera hora novamente após setDate
    }

    while (tempDateForRemainingDays <= lastDayOfMonth) {
        const dataStringTemp = formatDateToYYYYMMDD(tempDateForRemainingDays);
        const registroParaEsteDia = producoesDoMesVisualizado.find(p => p.data === dataStringTemp);
        const dayOfWeekTemp = tempDateForRemainingDays.getDay();
    ...
    tempDateForRemainingDays.setHours(0,0,0,0); // Zera a hora
    if (tempDateForRemainingDays < inicioSemana) { // Se a semana visualizada é futura
        tempDateForRemainingDays.setTime(inicioSemana.getTime());
    } else if (tempDateForRemainingDays > finalSemana) { // Se a semana visualizada já passou
        diasOperacaoRestantesSemana = 0;
    } else { // Semana atual
        tempDateForRemainingDays.setDate(dataAtualCalculo.getDate() + 1); // Começa a contar a partir de amanhã
        tempDateForRemainingDays.setHours(0,0,0,0); // Zera hora novamente após setDate
    }
