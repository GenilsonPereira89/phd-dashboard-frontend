...
async function atualizarDashboard(ano, mes) {
    ...
    let totalPessoasEmDiasOperacionaisQuinzena = 0;
    let metaQuinzenaTotal = 0;
        ...
        if (registroParaEsteDiaQuinzena) {
            if (!registroParaEsteDiaQuinzena.isExcecao) {
                ...
                if (tempDateForQuinzenaMeta <= dataAtualCalculo) {
                    ...
                    totalPessoasEmDiasOperacionaisQuinzena += (registroParaEsteDiaQuinzena.operadoresNoDia + registroParaEsteDiaQuinzena.diaristasNoDia);
                }
            }
        } else {
            if (dayOfWeekTemp !== 0) {
                metaQuinzenaTotal += getMetaDiariaParaDia(NUM_OPERADORES_PADRAO, 0);
            }
        }
    ...
    if (diasOperacaoRestantesQuinzena > 0) {
        ...
        if (producaoNecessariaParaMetaQuinzena > 0) {
            ...
            projecaoQuinzenaTexto = `Para atingir a meta da ${nomeQuinzena}, vocês precisam fazer uma média de ${Math.round(pacotesPorDiaNecessarios).toLocaleString('pt-BR')} pacotes por dia nos próximos ${diasOperacaoRestantesQuinzena} dias de operação. Isso exigiria aproximadamente **${pessoasNecessarias} operadores/diaristas** por dia, considerando o PHD de meta.`;
