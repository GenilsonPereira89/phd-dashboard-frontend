...
async function atualizarDashboard(ano, mes) {
    ...
    while (tempDateForMetaMensal <= ultimoDiaDoMes) {
        ...
        // 1. For um dia de semana (segunda a sábado)
        // 2. OU for um domingo E houver um registro para ele E ele NÃO for exceção (domingo trabalhado)
        if (dayOfWeekTemp !== 0) { // Todos os dias de semana (seg-sab) sempre contam
    ...
    if (diasRestantesParaProjecao > 0) {
        ...
        if (producaoAcumuladaMensal === 0 && diasQueContribuemParaMetaMensal === 0) { // Ajustado para usar diasQueContribuemParaMetaMensal
