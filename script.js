...
const metaMensalElement = document.getElementById('metaMensal'); // AGORA É A ÚNICA META MENSAL
...
// Calcula KPIs para um período específico (usado pelo Relatório Comparativo)
function calcularKpisParaPeriodo(producoesDoPeriodo, dataInicioPeriodo, dataFimPeriodo, numOperadoresPadrao) {
    ...
    let metaTotalPeriodo = 0; // Esta é a meta que deve ser igual à "Meta Mensal" do dashboard principal
    let diasOperacaoContabilizados = 0; // Dias que contribuem para a metaTotalPeriodo
    let totalOperadoresEmDiasOperacionais = 0; // Para cálculo do PHD
    ...
    finalDateForLoop.setHours(23,59,59,999);
    ...
    while (tempDate <= finalDateForLoop) {
        ...
        const dayOfWeekTemp = tempDate.getDay(); // 0 = Domingo

        // Lógica para a metaTotalPeriodo e diasOperacaoContabilizados (igual à Meta Mensal principal)
        if (dayOfWeekTemp !== 0) { // É um dia de semana (segunda a sábado)
            if (!registroParaEsteDia || !registroParaEsteDia.isExcecao) {
                // Se não há registro (dia futuro) ou se há registro e NÃO é exceção
                metaTotalPeriodo += getMetaDiariaParaDia(numOperadoresPadrao);
                diasOperacaoContabilizados++;
            }
        } else if (dayOfWeekTemp === 0 && registroParaEsteDia && !registroParaEsteDia.isExcecao) {
            // É um domingo E há um registro para ele E ele NÃO é exceção (Domingo Trabalhado)
            metaTotalPeriodo += getMetaDiariaParaDia(numOperadoresPadrao);
            diasOperacaoContabilizados++;
        }

        // Lógica para produção acumulada e operadores/diaristas reais (para PHD)
        if (registroParaEsteDia) {
            if (!registroParaEsteDia.isExcecao) {
                producaoAcumulada += registroParaEsteDia.producao;
                totalOperadoresEmDiasOperacionais += registroParaEsteDia.operadoresNoDia;
                totalDiaristas += registroParaEsteDia.diaristasNoDia;
            } else {
                // Mesmo se for exceção, diaristas podem ter sido registrados para rastreamento
                totalDiaristas += registroParaEsteDia.diaristasNoDia;
            }
        }
    ...
    const saldoTotal = producaoAcumulada - metaTotalPeriodo; 
    ...
    return {
        ...
        metaAcumulada: metaTotalPeriodo, // Retorna como metaAcumulada para ser usada no relatório
        saldoTotal: saldoTotal,
        phdMedio: phdMedio,
        diasOperacaoConsiderados: diasOperacaoContabilizados, 
...
gerarRelatorioComparativoBtn.addEventListener('click', async () => {
    ...
    const kpisParaComparar = [
        ...
        { id: 'metaAcumulada', label: 'Meta Total do Período (Pacotes)', format: 'number' }, // Esta é a meta que será ajustada
...
async function atualizarDashboard(ano, mes) {
    ...
    let diasQueContribuemParaMetaMensal = 0; // Total de dias úteis + domingos trabalhados para a meta do mês
    const tempDateForMetaMensal = new Date(primeiroDiaDoMes.getTime());
    while (tempDateForMetaMensal <= ultimoDiaDoMes) {
        const dataStringTemp = formatDateToYYYYMMDD(tempDateForMetaMensal);
        const registroParaEsteDia = producoesDoMesVisualizado.find(p => p.data === dataStringTemp);
        const dayOfWeekTemp = tempDateForMetaMensal.getDay(); // 0 = Domingo

        // Um dia contribui para a meta mensal se:
        // 1. For um dia de semana (segunda a sábado) E NÃO for exceção (seja por registro ou por padrão)
        // 2. OU for um domingo E houver um registro para ele E ele NÃO for exceção (domingo trabalhado)
        if (dayOfWeekTemp !== 0) { // Dias de semana (seg-sab)
            if (!registroParaEsteDia || !registroParaEsteDia.isExcecao) {
                // Se não há registro (dia futuro) ou se há registro e NÃO é exceção
                diasQueContribuemParaMetaMensal++;
            }
        } else if (dayOfWeekTemp === 0 && registroParaEsteDia && !registroParaEsteDia.isExcecao) {
            // Domingo só conta se for um registro de "Domingo Trabalhado"
            diasQueContribuemParaMetaMensal++;
        }
        tempDateForMetaMensal.setDate(tempDateForMetaMensal.getDate() + 1);
    }
    metaMensalCalculada = diasQueContribuemParaMetaMensal * NUM_OPERADORES_PADRAO * PACOTES_POR_OPERADOR_DIA_META;

    if (metaMensalElement) { 
        metaMensalElement.textContent = metaMensalCalculada.toLocaleString('pt-BR');
    }
    ...
    producoesDoMesVisualizado.forEach(registro => {
        ...
        if (!registro.isExcecao) {
            ...
            if (dataRegistroObj <= dataAtualCalculo) {
                ...
                // Conta dias de segunda a sábado OU domingos trabalhados ATÉ HOJE
    ...
    // A "Falta para a Meta Mensal" agora compara com a metaMensalCalculada
    const faltaParaMetaMensal = metaMensalCalculada - producaoAcumuladaMensal;
    faltaParaMetaMensalElement.textContent = faltaParaMetaMensal.toLocaleString('pt-BR');

    // --- Dias Restantes Mensal (Para a projeção da Meta Mensal) ---
    ...
    while (tempDateForRemainingDaysMonthly <= lastDayOfMonth) {
        ...
        // Um dia restante conta para a projeção se:
        // 1. For um dia de semana (segunda a sábado) E não for um dia de exceção já lançado
        // 2. OU for um domingo E for um dia de "Domingo Trabalhado" já lançado
        if (dayOfWeekTemp !== 0 && (!registroParaEsteDia || !registroParaEsteDia.isExcecao)) {
    ...
    // --- Projeção Mensal (agora baseada na metaMensalCalculada) ---
    if (diasRestantesParaProjecao > 0) {
        ...
        if (producaoAcumuladaMensal === 0 && diasQueContribuemParaMetaMensal === 0) { 
            projecaoTextoMensal = `Ainda não há lançamentos de produção para ${nomeMes} de ${anoVisualizado}. A meta para o mês é de ${metaMensalCalculada.toLocaleString('pt-BR')} pacotes. Comece a registrar a produção!`;
            projecaoTextoElement.style.color = 'blue';
        } else {
            const producaoNecessariaParaMetaMensal = metaMensalCalculada - producaoAcumuladaMensal; 

            if (producaoNecessariaParaMetaMensal > 0) {
                const pacotesPorDiaNecessarios = producaoNecessariaParaMetaMensal / diasRestantesParaProjecao;
                const pessoasNecessarias = Math.ceil(pacotesPorDiaNecessarios / PACOTES_POR_OPERADOR_DIA_META);

                projecaoTextoMensal = `Para atingir a Meta Mensal, vocês precisam fazer uma média de ${Math.round(pacotesPorDiaNecessarios).toLocaleString('pt-BR')} pacotes por dia nos próximos ${diasRestantesParaProjecao} dias de operação. Isso exigiria aproximadamente **${pessoasNecessarias} operadores** por dia, considerando o PHD de meta.`;
                projecaoTextoElement.style.color = 'red';
            } else {
                projecaoTextoMensal = `A Meta Mensal foi atingida ou superada! Com a produção atual, a projeção é de superar a meta em ${Math.abs(producaoNecessariaParaMetaMensal).toLocaleString('pt-BR')} pacotes! Continuem assim!`;
                projecaoTextoElement.style.color = 'green';

                const pacotesFaltantesParaMetaMensal = metaMensalCalculada - producaoAcumuladaMensal;
