...
async function atualizarDashboard(ano, mes) {
    ...
    // --- CÁLCULO DA META MENSAL TOTAL (FIXA) ---
    ...
    for (let i = 1; i <= ultimoDiaDoMes; i++) {
        ...
        // Conta apenas dias de segunda a sábado como dias úteis padrão para a meta fixa
        if (dayOfWeek !== 0) { // 0 = Domingo
            diasUteisNoMes++;
        }
    }
    // A meta mensal total é fixa, baseada nos dias úteis e operadores padrão
    metaMensalTotal = diasUteisNoMes * getMetaDiariaParaDia(NUM_OPERADORES_PADRAO);
    ...
    producoesDoMesVisualizado.forEach(registro => {
        ...
        const totalPessoasNoDia = registro.operadoresNoDia + registro.diaristasNoDia; // Total de pessoas para o PHD da tabela
        // PHD Diário: Produção / (Operadores) do dia (se operadores > 0)
    ...
    // Calcula dias restantes no mês (apenas dias úteis padrão)
    if (anoVisualizado === dataAtualCalculo.getFullYear() && mesVisualizado === (dataAtualCalculo.getMonth() + 1)) {
        ...
        // A meta esperada até hoje já considera os lançamentos, então para dias restantes, contamos os dias úteis padrão
        let diasUteisNaoLancados = 0;
        ...
        while(tempDate <= fimMes) {
            ...
            if (tempDate.getDay() !== 0 && !registroParaEsteDia) { // Se for dia útil e não lançado
                diasUteisNaoLancados++;
            }
            tempDate.setDate(tempDate.getDate() + 1);
        }
        diasRestantesMensal = diasUteisNaoLancados;
    }
    ...
    // PHD Médio Mensal (baseado na produção e total de OPERADORES DOS DIAS CONSIDERADOS)
    ...
    if (diasRestantesMensal > 0) {
        ...
        if (producaoAcumuladaMensal === 0 && diasDeOperacaoConsideradosMensal === 0) {
            ...
            if (projecaoSuperar < 0) {
                ...
                // Evita divisão por zero se NUM_OPERADORES_PADRAO for 0
    ...
    // --- CÁLCULOS E PROJEÇÕES QUINZENAIS ---
    let dataInicioQuinzena;
    let dataFimQuinzena;
    let nomeQuinzena;
    let diasUteisNaQuinzena = 0; // Para a meta fixa da quinzena
    ...
    if (dataAtualCalculo.getDate() <= 15) {
        ...
        dataFimQuinzena = new Date(anoVisualizado, mesVisualizado, 0); // Último dia do mês
        nomeQuinzena = "2ª Quinzena";
    }

    // Calcula dias úteis para a meta fixa da quinzena
    const tempDateForQuinzenaDays = new Date(dataInicioQuinzena.getTime());
    while (tempDateForQuinzenaDays <= dataFimQuinzena) {
        if (tempDateForQuinzenaDays.getDay() !== 0) { // Não é domingo
            diasUteisNaQuinzena++;
        }
        tempDateForQuinzenaDays.setDate(tempDateForQuinzenaDays.getDate() + 1);
    }
    ...
    // A metaQuinzenaTotal agora é fixa, baseada nos dias úteis e operadores padrão
    let metaQuinzenaTotal = diasUteisNaQuinzena * getMetaDiariaParaDia(NUM_OPERADORES_PADRAO);
    ...
    producoesDaQuinzena.forEach(registro => {
        const operadoresCalculo = registro.isExcecao ? 0 : registro.operadoresNoDia;
        const metaDiariaDoRegistro = getMetaDiariaParaDia(operadoresCalculo);

        if (!registro.isExcecao && new Date(registro.data + 'T00:00:00') <= dataAtualCalculo) { // Apenas dias passados/hoje
            producaoAcumuladaQuinzena += registro.producao;
            metaAcumuladaQuinzena += metaDiariaDoRegistro;
            diasOperacaoConsideradosQuinzena++;
            totalOperadoresEmDiasOperacionaisQuinzena += registro.operadoresNoDia;
        }
    });
    ...
    let diasUteisNaSemana = 0; // Para a meta fixa da semana
    const tempDateForWeeklyDays = new Date(inicioSemana.getTime());
    while (tempDateForWeeklyDays <= finalSemana) {
        if (tempDateForWeeklyDays.getDay() !== 0) { // Não é domingo
            diasUteisNaSemana++;
        }
        tempDateForWeeklyDays.setDate(tempDateForWeeklyDays.getDate() + 1);
    }
    let metaSemanalTotal = diasUteisNaSemana * getMetaDiariaParaDia(NUM_OPERADORES_PADRAO);
    ...
    producoesDaSemana.forEach(registro => {
        const operadoresCalculo = registro.isExcecao ? 0 : registro.operadoresNoDia;
        const metaDiariaDoRegistro = getMetaDiariaParaDia(operadoresCalculo);

        if (!registro.isExcecao && new Date(registro.data + 'T00:00:00') <= dataAtualCalculo) { // Apenas dias passados/hoje
            producaoAcumuladaSemanal += registro.producao;
            metaAcumuladaSemanal += metaDiariaDoRegistro;
            diasOperacaoConsideradosSemanal++;
            totalOperadoresEmDiasOperacionaisSemanal += registro.operadoresNoDia;
        }
    });
