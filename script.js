...
// A meta diária agora considera APENAS os operadores, não os diaristas.
...
async function atualizarDashboard(ano, mes) {
    ...
    // --- CÁLCULO DA META MENSAL TOTAL (AGORA DINÂMICA) ---
    // Esta lógica irá recalcular a meta total do mês com base nos lançamentos existentes
    // e no padrão para os dias não lançados.
    ...
    for (let i = 1; i <= ultimoDiaDoMes; i++) {
        ...
        const dataString = `${dataIteracao.getFullYear()}-${String(dataIteracao.getMonth() + 1).padStart(2, '0')}-${String(dataIteracao.getDate()).padStart(2, '0')}`;
        const dayOfWeek = dataIteracao.getDay(); // 0 = Domingo

        const registroParaEsteDia = producoesDoMesVisualizado.find(p => p.data === dataString);

        if (registroParaEsteDia) {
            // Se há registro, usa os dados do registro
            if (!registroParaEsteDia.isExcecao) { // Se não é um dia de exceção
                metaMensalTotal += getMetaDiariaParaDia(registroParaEsteDia.operadoresNoDia);
            }
            // Se for exceção, a meta para o dia é 0, então não adiciona nada
        } else {
            // Se não há registro, usa o padrão para dias úteis
            if (dayOfWeek !== 0) { // Se não for domingo
                metaMensalTotal += getMetaDiariaParaDia(NUM_OPERADORES_PADRAO);
            }
            // Se for domingo e não tem registro, a meta para o dia é 0
        }
    }
    ...
    producoesDoMesVisualizado.forEach(registro => {
        ...
        const metaDiariaDoRegistro = getMetaDiariaParaDia(operadoresCalculo);
    ...
    // Calcula dias restantes no mês que ainda são "operáveis" (não lançados como exceção e não domingos não trabalhados)
    if (anoVisualizado === dataAtualCalculo.getFullYear() && mesVisualizado === (dataAtualCalculo.getMonth() + 1)) {
        // Se estamos no mês atual, contamos a partir de amanhã até o final do mês
        for (let i = dataAtualCalculo.getDate() + 1; i <= ultimoDiaDoMes; i++) {
            const dataIteracao = new Date(anoVisualizado, mesVisualizado - 1, i);
            const dataString = `${dataIteracao.getFullYear()}-${String(dataIteracao.getMonth() + 1).padStart(2, '0')}-${String(dataIteracao.getDate()).padStart(2, '0')}`;
            const registroParaEsteDia = producoesDoMesVisualizado.find(p => p.data === dataString);

            // Considera o dia como restante se não foi lançado ou se foi lançado e NÃO é exceção
        ...
        // Se estamos visualizando um mês passado ou futuro, contamos os dias úteis que não foram lançados como exceção
        const inicioMes = new Date(anoVisualizado, mesVisualizado - 1, 1);
        const fimMes = new Date(anoVisualizado, mesVisualizado, 0);
        const tempDate = new Date(inicioMes.getTime());
        while(tempDate <= fimMes) {
            const dataString = `${tempDate.getFullYear()}-${String(tempDate.getMonth() + 1).padStart(2, '0')}-${String(tempDate.getDate()).padStart(2, '0')}`;
            const registroParaEsteDia = producoesDoMesVisualizado.find(p => p.data === dataString);
            
            // Se é um dia útil (não domingo) e não há registro OU há registro e NÃO é exceção
            if (tempDate.getDay() !== 0 && (!registroParaEsteDia || !registroParaEsteDia.isExcecao)) {
                diasRestantesMensal++;
            }
            tempDate.setDate(tempDate.getDate() + 1);
        }
        // Para meses passados/futuros, os "dias restantes" para projeção são os dias operacionais não lançados
        // que ainda não foram considerados na meta acumulada.
        // A meta acumulada já considera os dias operacionais lançados.
        // Então, os dias restantes para projeção são os dias operacionais totais do mês (calculados na metaMensalTotal)
        // menos os dias operacionais já passados/lançados.
        
        // Vamos recalcular dias operacionais totais do mês para ter certeza
        let totalDiasOperacionaisNoMes = 0;
        const tempDateTotal = new Date(anoVisualizado, mesVisualizado - 1, 1);
        while (tempDateTotal <= new Date(anoVisualizado, mesVisualizado, 0)) {
            const dataStringTemp = `${tempDateTotal.getFullYear()}-${String(tempDateTotal.getMonth() + 1).padStart(2, '0')}-${String(tempDateTotal.getDate()).padStart(2, '0')}`;
            const registroTemp = producoesDoMesVisualizado.find(p => p.data === dataStringTemp);
            if (registroTemp) {
                if (!registroTemp.isExcecao) {
                    totalDiasOperacionaisNoMes++;
                }
            } else {
                if (tempDateTotal.getDay() !== 0) { // Se não é domingo e não tem registro
                    totalDiasOperacionaisNoMes++;
                }
            }
            tempDateTotal.setDate(tempDateTotal.getDate() + 1);
        }
        diasRestantesMensal = totalDiasOperacionaisNoMes - diasDeOperacaoConsideradosMensal;
    }
    ...
    // --- CÁLCULOS E PROJEÇÕES QUINZENAIS (AGORA DINÂMICAS) ---
    ...
    let metaQuinzenaTotal = 0; // Agora dinâmica
    ...
    while (tempDateForQuinzenaMeta <= dataFimQuinzena) {
        ...
        if (registroParaEsteDiaQuinzena) {
            if (!registroParaEsteDiaQuinzena.isExcecao) {
                metaQuinzenaTotal += getMetaDiariaParaDia(registroParaEsteDiaQuinzena.operadoresNoDia);
                if (tempDateForQuinzenaMeta <= dataAtualCalculo) {
                    producaoAcumuladaQuinzena += registroParaEsteDiaQuinzena.producao;
                    metaAcumuladaQuinzena += getMetaDiariaParaDia(registroParaEsteDiaQuinzena.operadoresNoDia);
    ...
    while (tempDateForRemainingQuinzenaDays <= dataFimQuinzena) {
        ...
        if (!registroParaEsteDiaQuinzenaFuturo || !registroParaEsteDiaQuinzenaFuturo.isExcecao) {
            if (tempDateForRemainingQuinzenaDays.getDay() !== 0) {
    ...
    // --- CÁLCULOS E PROJEÇÕES SEMANAIS (AGORA DINÂMICAS) ---
    ...
    let metaSemanalTotal = 0; // Agora dinâmica
    ...
    while (tempDateForWeeklyMeta <= finalSemana) {
        ...
        if (registroParaEsteDiaSemana) {
            if (!registroParaEsteDiaSemana.isExcecao) {
                metaSemanalTotal += getMetaDiariaParaDia(registroParaEsteDiaSemana.operadoresNoDia);
                if (tempDateForWeeklyMeta <= dataAtualCalculo) {
                    producaoAcumuladaSemanal += registroParaEsteDiaSemana.producao;
                    metaAcumuladaSemanal += getMetaDiariaParaDia(registroParaEsteDiaSemana.operadoresNoDia);
