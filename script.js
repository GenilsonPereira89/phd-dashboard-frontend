...
async function atualizarDashboard(ano, mes) {
    ...
    let tempDateForRemainingDaysMonthly = new Date(today.getTime()); // RENOMEADO AQUI
    tempDateForRemainingDaysMonthly.setHours(0,0,0,0); // Garante que a hora seja 00:00:00

    if (tempDateForRemainingDaysMonthly < primeiroDiaDoMes) { // Se o mês visualizado é futuro
        tempDateForRemainingDaysMonthly.setTime(primeiroDiaDoMes.getTime());
    } else if (tempDateForRemainingDaysMonthly > lastDayOfMonth) { // Se o mês visualizado já passou
        diasRestantesParaProjecao = 0;
    } else { // Mês atual
        tempDateForRemainingDaysMonthly.setDate(today.getDate() + 1); // Começa a contar a partir de amanhã
        tempDateForRemainingDaysMonthly.setHours(0,0,0,0); // Zera hora novamente após setDate
    }
    ...
    while (tempDateForRemainingDaysMonthly <= lastDayOfMonth) {
        ...
        // Se não há registro OU se há registro e NÃO é exceção E NÃO é domingo
    ...
    let tempDateForRemainingQuinzenaDays = new Date(dataAtualCalculo.getTime()); // Renomeado para evitar conflito
    tempDateForRemainingQuinzenaDays.setHours(0,0,0,0); // Zera a hora
    if (tempDateForRemainingQuinzenaDays < dataInicioQuinzena) { // Se a quinzena visualizada é futura
        tempDateForRemainingQuinzenaDays.setTime(dataInicioQuinzena.getTime());
    } else if (tempDateForRemainingQuinzenaDays > dataFimQuinzena) { // Se a quinzena visualizada já passou
        diasOperacaoRestantesQuinzena = 0;
    } else { // Quinzena atual
        tempDateForRemainingQuinzenaDays.setDate(dataAtualCalculo.getDate() + 1); // Começa a contar a partir de amanhã
        tempDateForRemainingQuinzenaDays.setHours(0,0,0,0); // Zera hora novamente após setDate
    }
    ...
    let tempDateForRemainingWeeklyDays = new Date(dataAtualCalculo.getTime()); // RENOMEADO AQUI
    tempDateForRemainingWeeklyDays.setHours(0,0,0,0); // Zera a hora
    if (tempDateForRemainingWeeklyDays < inicioSemana) { // Se a semana visualizada é futura
        tempDateForRemainingWeeklyDays.setTime(inicioSemana.getTime());
    } else if (tempDateForRemainingWeeklyDays > finalSemana) { // Se a semana visualizada já passou
        diasOperacaoRestantesSemana = 0;
    } else { // Semana atual
        tempDateForRemainingWeeklyDays.setDate(dataAtualCalculo.getDate() + 1); // Começa a contar a partir de amanhã
        tempDateForRemainingWeeklyDays.setHours(0,0,0,0); // Zera hora novamente após setDate
    }
