...
async function atualizarDashboard(ano, mes) {
    ...
    for (let i = 1; i <= ultimoDiaDoMes; i++) {
        ...
        const dayOfWeek = dataAtualIteracao.getDay();
        ...
        if (registroParaEsteDia) {
            ...
            // Se o dia não foi lançado e não é domingo, usa o padrão
    ...
    for (let i = hojeData.getDate() + 1; i <= ultimoDiaDoMes; i++) {
        const dataIteracao = new Date(anoVisualizado, mesVisualizado - 1, i);
        if (dataIteracao.getDay() !== 0) { // Se não for domingo
            diasRestantesMensal++;
        }
    }
    // Ajusta se o mês visualizado não é o mês atual
    if (anoVisualizado !== hojeData.getFullYear() || mesVisualizado !== (hojeData.getMonth() + 1)) {
    ...
    const hoje = new Date();
    // Ajusta a data de hoje para o início do dia para comparações
    hoje.setHours(0, 0, 0, 0);

    // Encontra o início da semana (segunda-feira)
    const diaDaSemanaHoje = hoje.getDay(); // 0 = Domingo, 1 = Segunda, ..., 6 = Sábado
    const inicioSemana = new Date(hoje);
    inicioSemana.setDate(hoje.getDate() - (diaDaSemanaHoje === 0 ? 6 : diaDaSemanaHoje - 1)); // Se for domingo, volta 6 dias. Senão, volta (dia-1) dias.
    inicioSemana.setHours(0, 0, 0, 0);

    // Encontra o final da semana (domingo)
    ...
    // CÁLCULO DA META SEMANAL TOTAL E KPIs ACUMULADOS DA SEMANA
    ...
    while (tempDateForWeeklyMeta <= finalSemana) {
        ...
        if (registroParaEsteDiaSemana) {
            if (!registroParaEsteDiaSemana.isExcecao) {
                ...
                // Acumula para KPIs da semana apenas se o dia já passou ou é hoje
                if (tempDateForWeeklyMeta <= hoje) {
            ...
            // Se o dia não foi lançado e não é domingo, usa o padrão para meta futura
            if (dayOfWeekTemp !== 0) { // Domingo é 0
    ...
    // Calcula dias restantes de operação na semana (apenas dias úteis padrão)
    let diasOperacaoRestantesSemana = 0;
    const tempDateForRemainingDays = new Date(hoje.getTime());
    tempDateForRemainingDays.setDate(hoje.getDate() + 1); // Começa a partir de amanhã
    while (tempDateForRemainingDays <= finalSemana) {
        ...
        if (!registroParaEsteDiaSemanaFuturo || !registroParaEsteDiaSemanaFuturo.isExcecao) { // Se não foi lançado ou não é exceção
    ...
    // --- Projeção Semanal e Recomendação de Operadores/Diaristas ---
    ...
    if (diasOperacaoRestantesSemana > 0) {
        ...
        // Se não há dias restantes na semana (semana encerrada)
        if (saldoSemanal < 0) {
